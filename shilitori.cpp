...
std::map<std::string, std::vector<std::string>> light_dict = cut_word(word_dict);

// 要素数をカウント
unsigned int count_words() {
    unsigned int count = 0;
    for(const auto &item : light_dict) {
...
// 終了判定
bool judge_end(std::string pword, std::string pwe, std::string cwe) {
    if(pwe == "ン") {
        std::cout << "「ン」が付きました" << std::endl << "あなたの負けです" << std::endl
                  << "お疲れ様でした" << std::endl << std::endl;
    } else if(light_dict[pwe].size() == 0) {
...
int main() {
    ...
    std::cout << "（ 合計単語数：" << count_words() << "、最小単語数：" << minor_words() << " ）" << std::endl << std::endl;
    ...
    while(true) {
        ...
        light_dict[cwe] = remove_element(light_dict[cwe], pword);
        if(judge_error(pword, pwf, cwe, occur)) {
            continue;
        } else if(judge_end(pword, pwe, cwe)) {
            break;
        }
        occur.push_back(pword);
        std::string cword = choice_random(light_dict[pwe]);
        light_dict[pwe] = remove_element(light_dict[pwe], cword);
        occur.push_back(cword);
